    ...
    with driver.session() as session:
        try:
            ...
            if version.startswith("5"):
                ...
                session.run(
                    "CREATE INDEX IF NOT EXISTS FOR (p:Page) ON (p.timestamp)")
                session.run(
                    "CREATE INDEX IF NOT EXISTS FOR (p:Page) ON (p.session_id)")
            # Neo4j 4.x syntax
            elif version.startswith("4"):
                # Create constraints using Neo4j 4.x syntax
                session.run(
                    "CREATE CONSTRAINT IF NOT EXISTS ON (p:Page) ASSERT p.url IS UNIQUE")
                session.run(
                    "CREATE CONSTRAINT IF NOT EXISTS ON (f:Form) ASSERT f.id IS UNIQUE")
            ...
            try:
                ...
                session.run(
                    "CREATE INDEX index_page_url IF NOT EXISTS FOR (p:Page) ON (p.url)")
...
def get_page_details(url):
    ...
    with driver.session() as session:
        try:
            # First, get the page and basic form data
            result = session.run("""
            MATCH (p {url: $url})
            ...
            for form in forms:
                if form:  # Some forms might be null
                    ...
                    MATCH (f:Form {id: $form_id})-[:HAS_FIELD]->(fld:FormField)
                    RETURN fld
                    ORDER BY fld.field_index
                    """, form_id=form.get("id", ""))
                    ...
                    for field_record in field_result:
                        if field_record["fld"]:
                            field_relationships.append({
                                "end": dict(field_record["fld"].items())
                            })

                    # Add field relationships to form data
                    if field_relationships:
                        form_data["HAS_FIELD"] = field_relationships
                        print(
                            f"✅ Found {len(field_relationships)} field relationships for form {form.get('id', '')}")
                    else:
                        print(
                            f"❌ No field relationships found for form {form.get('id', '')}")
                        # Try to get standalone field data if relationships don't exist
                        standalone_result = session.run("""
                        MATCH (p {url: $url})-[:HAS_STANDALONE_FIELD]->(sf:StandaloneField)
                        RETURN sf
                        ORDER BY sf.field_index
                        """, url=url)

                        standalone_fields = []
                        for sf_record in standalone_result:
                            if sf_record["sf"]:
                                standalone_fields.append(
                                    dict(sf_record["sf"].items()))

                        if standalone_fields:
                            # Convert standalone fields to form field format
                            form_data["fields"] = standalone_fields
                            print(
                                f"✅ Found {len(standalone_fields)} standalone fields converted to form fields")
            ...
            if "timestamp" in properties:
                try:
                    ...
                    if isinstance(timestamp, int):
                        ...
                        properties["timestamp_readable"] = dt.strftime(
                            "%Y-%m-%d %H:%M:%S")
                except:
                    pass
            ...
            print(f"Error getting enhanced page details: {e}")
            return None
...
def get_capture_stats(session_id=None):
    ...
    with driver.session() as session:
        try:
            # Get counts of different node types and enhanced field statistics
            if session_id:
                # Filter by session
                result = session.run("""
                MATCH (p:Page)
                ...
                WITH count(p) as page_count,
                     sum(p.total_fields) as total_fields,
                     sum(p.named_fields) as named_fields,
                     sum(p.id_fields) as id_fields,
                     sum(p.placeholder_fields) as placeholder_fields,
                     sum(p.required_fields) as required_fields,
                     sum(p.aria_labeled_fields) as aria_labeled_fields
                OPTIONAL MATCH (f:Form)
                WHERE f.session_id = $session_id
                WITH page_count, total_fields, named_fields, id_fields, placeholder_fields, 
                     required_fields, aria_labeled_fields, count(f) as form_count
                OPTIONAL MATCH (a:Alert)
                WHERE a.session_id = $session_id
                WITH page_count, form_count, total_fields, named_fields, id_fields, 
                     placeholder_fields, required_fields, aria_labeled_fields, count(a) as alert_count
                OPTIONAL MATCH ()-[r:LEADS_TO]->()
                RETURN page_count, form_count, alert_count, count(r) as flow_count,
                       total_fields, named_fields, id_fields, placeholder_fields,
                       required_fields, aria_labeled_fields
                """, session_id=session_id)
            else:
                # Get all stats
                result = session.run("""
            ...
            return {
                "pages": record["page_count"] or 0,
                "forms": record["form_count"] or 0,
                "alerts": record["alert_count"] or 0,
                "flows": record["flow_count"] or 0,
                "total_fields": record["total_fields"] or 0,
                "named_fields": record["named_fields"] or 0,
                "id_fields": record["id_fields"] or 0,
                "placeholder_fields": record["placeholder_fields"] or 0,
                "required_fields": record["required_fields"] or 0,
                "aria_labeled_fields": record["aria_labeled_fields"] or 0
            }
        except Exception as e:
